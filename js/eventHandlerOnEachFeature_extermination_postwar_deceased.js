...
function resetHighlight_extermination_postwar_deceased (event) {
   ...
   jsonLayer_extermination_camps_postwar_deceased.resetStyle(event.target);
};

function eventHandlerOnEachFeature_extermination_postwar_deceased(feature, layer) {
    ...
    layer.on({
         ...
         'mouseout': resetHighlight_extermination_postwar_deceased
     });
    ...
    layer.bindPopup(
      ...
      '<p>'+'<strong>'+feature.properties.quantity+'</strong>'+' persone dal nostro dataset sono decedute nel campo prima della chiusura '+
      '<strong>'+feature.properties.convoyArrivalDate2+'</strong>'+'</p>'
    );
};

map.createPane('pane_jsonLayer_extermination_camps_postwar_deceased');
map.getPane('pane_jsonLayer_extermination_camps_postwar_deceased').style.zIndex = 405;
map.getPane('pane_jsonLayer_extermination_camps_postwar_deceased').style['mix-blend-mode'] = 'normal';

var jsonLayer_extermination_camps_postwar_deceased =  L.geoJSON(extermination_camps_postwar_deceased, {
  pane: 'pane_jsonLayer_extermination_camps_postwar_deceased',//set the style for the layer, using the function name present in layer_style.js
  style: style_extermination_camps_postwar_deceased,
  ...
  onEachFeature: eventHandlerOnEachFeature_extermination_postwar_deceased,
  ...
  pointToLayer: function (feature, latlng) {
    return L.circleMarker(latlng,{
     ...
     radius: (Math.log2(feature.properties.quantity)+1)*4
   })
  }
});
// We pass the variable jsonLayerPublications, created earlier, to the timeDimension method to manage geoJSON
var jsonLayer_extermination_camps_postwar_deceased_time =  L.TimeDimension.Layer.betweendates (jsonLayer_extermination_camps_postwar_deceased, {
...
jsonLayer_extermination_camps_postwar_deceased_time.addTo(map);
