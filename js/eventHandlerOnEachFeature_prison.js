...
function resetHighlight_prison (event) {
   ...
   jsonLayer_prison_camps.resetStyle(event.target);
};

function eventHandlerOnEachFeature_prison(feature, layer) {
    ...
    layer.on({
         ...
         'mouseout': resetHighlight_prison
     });
    ...
    layer.bindPopup(
      '<h3>'+'<strong>'+feature.properties.detentionPlace+'</strong>'+'</h3>'+
      '<p>'+'<strong>'+feature.properties.quantity+'</strong>'+' persone dal nostro dataset sono state internate nella prigione tra '+
      '<strong>'+feature.properties.arrestDate_label+'</strong>'+' e '+'<strong>'+feature.properties.arrestDateEnd_label
      +'</strong>'+'</p>'
    );
};

map.createPane('pane_jsonLayer_prison_camps');
map.getPane('pane_jsonLayer_prison_camps').style.zIndex = 402;
map.getPane('pane_jsonLayer_prison_camps').style['mix-blend-mode'] = 'normal';

var jsonLayer_prison_camps =  L.geoJSON(prison_camps, {
  pane: 'pane_jsonLayer_prison_camps',
  //set the style for the layer, using the function name present in layer_style.js
  style: style_prison_camps,
  ...
  onEachFeature: eventHandlerOnEachFeature_prison,
...
var jsonLayer_prison_camps_time =  L.TimeDimension.Layer.betweendates (jsonLayer_prison_camps, {
...
jsonLayer_prison_camps_time.addTo(map);
